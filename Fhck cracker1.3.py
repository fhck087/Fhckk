...
gg = 0
#Tool made by @mmhhn - @fhck
WDEH = render('Fhck', colors=['white', 'cyan'], align='center')
print(WDEH)
ID = input("id : ")
print('')
token = input("token : ")
def Users():
    ...
    try:
        ...
        with open("username.txt", "a") as file:
            file.write(username + "\n")
        Ex+=1
        sys.stdout.write(f"\r[{N}{Ex}{C}]{r}-> User Name : {username}{reset}\r")
        return username
...
def fs(id):
    ...
    for i in r.json()['users']:
        gg+=1
        userL = i['username']
        print(f"\r[{N}{gg}{C}]-> User Name : {userL}{reset}\r")
...
def fg(id):
    ...
    exit()	
print(f""" V1.2 \n
{r}[{N}1{C}]{r}{bbo} ->   {C}Check List{N} DN {r}
[{N}2{C}]{r}{bbo} -> {C}  Get List {N} DN  {r}
[{N}3{C}]{r}{bbo} ->   {C} Delet  {N} List {r}
[{N}4{C}]{r}{bbo} ->   {C} Dev {N} fhck {r}
""")
try:
  
 egi = int(input(f"{C}[{N}fhck{C}] {N}Enter choice : {r}"))
except:print('');exit(print(f'{C}[{N}+{C}] {N}Enter Correct Number'))
if egi == 1:
    try:requests.get(f"https://api.telegram.org/bot{token}/sendvideo?chat_id={ID}&video=https://t.me/xxvyx/5&caption=Welcome To Fhck area ..\n\nDev @tacdl - @mmmhn")
    except:pass
    pass
elif egi == 2:
  #Tool made by @mmhhn - @fhck
    print(f"""
    {r}[{N}1{C}] -> {C}  Get {N}2011
    [{N}2{C}] -> {C}  Get {N}2012
    [{N}3{C}] -> {C}  Get {N}2013
    [{N}4{C}] -> {C}  Get {N}14-23
    [{N}5{C}] -> {C}  Get {N} in User
    """)
    #Tool made by @mmhhn - @fhck
    ...
    if hh ==1:
      ...
      for thread in threads:
       ...
       windows = 'cls'
       print('') 
       username = ("dr.ardakatirciog")
       print('')
       password = ("yasen161998")
       print('')
...
namefile1 = str(input(f'{r}{C}[{N}→{C}]{N} Enter Name File username : '))
...
#Tool made by @mmhhn - @fhck
print(f"""
    {r}[{N}1{C}] -> {C}  Outlook{N} .com
    [{N}2{C}] -> {C}  Hotmail{N} .com
    [{N}3{C}] -> {C}  Gmail{N} .com
    """)
ik = int(input(f"{C}[{N}→{C}] {N}Enter your Choece : {r}"))
if ik == 1:
 def hot(email):
            global bads_email
            cookies = {
 ...
 def check(email):
  ...
  bi = random.randint(5,208)
  bos = f'\x1b[38;5;{bi}m'
  sys.stdout.write(f'''\r{bos}[{r}{bbo}{n} fhck {bos}{r} {bbo}{n}Hits:{r} {bbo}{G}{hits} {n}Bad IG:{r}{bbo} {R}{bads_instgram} {r}{bbo}{n}Bad Email: {r}{bbo}{X}{bads_email}{r}{bbo}{r}{bos} ]  \r''')
